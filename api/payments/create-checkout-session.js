...
const createStripeSession = async ({ plan, method }) => {
  ...
  if (!secretKey) {
    return {
      status: 501,
      body: {
        message: "Stripe secret key is not configured. Complete Stripe onboarding and set STRIPE_SECRET_KEY.",
      },
    };
  }
  ...
  if (!selectedPlan || !stripeMethod) {
    return { status: 400, body: { message: "Invalid plan or payment method" } };
  }
  ...
  if (!response.ok) {
    return { status: response.status, body: { message: result.error?.message || "Stripe checkout failed" } };
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (provider !== "stripe") {
      res.status(501).json({
        message:
          "This provider is not implemented yet. Use Stripe first, or add LianLian/PingPong API credentials and adapter code.",
      });
      return;
    }
