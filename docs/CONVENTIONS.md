...
8. 校验层(样板 `assets/validate.js`,UMD 前后端共用):API 响应须经契约校验(`catalogResponse` 守护结构/类型,原样返回服务端已净化数据,不重写字段);客户端统一经 `assets/catalog.js` 式封装(超时/Abort/错误映射)。**错误契约分层**:协议级错误(401/403/405/429)保持顶层 `{ ok:false, code, message }`;业务级错误(400/404/413/415/503 等)统一 `{ ok:false, error:{ code, message } }`(经 `validate.apiError`)。已推广:sync/blob-upload/payments/paypal/catalog;ximalaya/email/wechat 暂保持旧契约,启用前改造。
